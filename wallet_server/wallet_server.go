...
import (
	...
	"log"
	"net/http"
	"newgoblockchain/block"
	"newgoblockchain/utils"
	"newgoblockchain/wallet"
	"path"
	"strconv"
)

const tempDir = "C:/Users/sheyi/Projects/Go/src/newgoblockchain/wallet_server/templates"
...
func (ws *WalletServer) CreateTransaction(w http.ResponseWriter, r *http.Request) {
	...
	case http.MethodPost:
		...
		if err != nil {
			...
			io.WriteString(w, string(utils.JsonStatus("fail 1")))
			return
		}
		if !t.Validate() {
			log.Println("ERROR: missing field(s)")
			io.WriteString(w, string(utils.JsonStatus("fail 2")))
			return
		}
		...
		io.WriteString(w, string(utils.JsonStatus("fail 4")))
